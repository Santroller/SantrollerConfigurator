...
public partial class JoystickToDpad : Output
{
    ...
    public JoystickToDpad(ConfigViewModel model,bool enabled,  bool peripheral, int threshold, bool wii) : base(
        model, enabled, new JoystickToDpadInput(model), Colors.Black, Colors.Black, [], [], [],
    ...
    public int Threshold
    {
        ...
        set
        {
            this.RaiseAndSetIfChanged(ref _threshold, value);
            foreach (var controllerButton in _outputs)
            {
                (controllerButton.Input as AnalogToDigital)!.Threshold = value;
            }
        }
    }
