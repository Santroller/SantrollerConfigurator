...
[ProtoContract]
public class SerializedGuitarButton : SerializedOutput
{
    public SerializedGuitarButton()
    {
        LedIndex = [];
        LedIndexMpr121 = [];
        LedIndexPeripheral = [];
    }
    public SerializedGuitarButton(SerializedInput input, bool enabled, Color ledOn, Color ledOff, byte[] ledIndex, byte[] ledIndexPeripheral, int debounce,
        InstrumentButtonType type, bool outputEnabled, int outputPin, bool outputInverted, bool outputPeripheral, bool childOfCombined, byte[] ledIndexMpr121)
    {
        Enabled = enabled;
        Input = input;
    ...
    public override Output Generate(ConfigViewModel model)
    {
        ...
        var combined = new GuitarButton(model, Enabled,Input.Generate(model), Color.FromUInt32(LedOn),
            Color.FromUInt32(LedOff), LedIndex, LedIndexPeripheral, LedIndexMpr121, Debounce, Type, OutputEnabled, OutputPeripheral, OutputInverted, OutputPin, ChildOfCombined);
