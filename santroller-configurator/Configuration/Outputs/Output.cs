...
public abstract partial class Output : ReactiveObject
{
        ...
        bool childOfCombined)
    {
        ...
        _isWs2812Helper = this.WhenAnyValue(x => x.Model.LedType).Select(x => x.IsWs2812())
            .ToProperty(this, x => x.IsWs2812);
        _isWs2812PeripheralHelper = this.WhenAnyValue(x => x.Model.LedTypePeripheral).Select(x => x.IsWs2812())
        ...
        if (this is OutputButton or {Input.IsAnalog: false} and not Led)
        {
    ...
    public IEnumerable<ProKeyType> ProKeyTypes => Enum.GetValues<ProKeyType>();
        ...
        UsbHostInputType? usbInputType, AccelInputType? accelInputType, MidiType? midiType)
    {
        ...
        switch (inputType)
        {
            ...
            case InputType.EncoderInput:
                input = new EncoderInput(-1, 1, false, Model);
                break;
            case InputType.EncoderPeripheralInput:
                input = new EncoderInput(-1, 1, true, Model);
                break;
            case InputType.ConstantInput:
        ...
        ReadOnlySpan<byte> peripheralQuadRaw)
    {
        ...
        if (Enabled)
            Input.Update(analogRaw, digitalRaw, ps2Raw, wiiRaw, djLeftRaw, djRightRaw, gh5Raw,
                ...
                cloneRaw, adxlRaw, mpr121Raw, midiRaw, bluetoothInputsRaw, peripheralConnected, crkdRaw, peripheralQuadRaw);
        ...
        foreach (var output in AllOutputs)
            if (output != this)
                output.Update(analogRaw, digitalRaw, ps2Raw, wiiRaw, djLeftRaw, djRightRaw, gh5Raw,
                    ...
                    digitalPeripheral, cloneRaw, adxlRaw, mpr121Raw, midiRaw, bluetoothInputsRaw, peripheralConnected, crkdRaw, peripheralQuadRaw);
    }
