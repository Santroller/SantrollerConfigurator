...
public class WiiInput : TwiInput
{
    ...
    private static readonly Dictionary<WiiInputType, string> Mappings = new()
    {
        ...
        {WiiInputType.NunchukRotationPitch, "fxpt_atan2s(accY,accZ)"},
        {WiiInputType.NunchukRotationRoll, "fxpt_atan2s(accX,accZ)"},
        ...
        {WiiInputType.GuitarPedal, "((wiiButtonsHigh) & (1 << 2))"},
        ...
        switch (checkedType)
        {
            ...
            case WiiControllerType.Guitar:
                ...
                RawValue = Input switch
                {
                    ...
                    WiiInputType.GuitarTapYellow => lastTapWii.HasFlag(BarButton.Yellow) ? 1 : 0,
                    WiiInputType.GuitarTapBlue => lastTapWii.HasFlag(BarButton.Blue) ? 1 : 0,
                    WiiInputType.GuitarTapOrange => lastTapWii.HasFlag(BarButton.Orange) ? 1 : 0,
                    _ => RawValue
                };
                break;
            ...
            case WiiControllerType.Dj:
                RawValue = Input switch
                {
                    ...
                    WiiInputType.DjTurntableLeft =>
                        ((((wiiData[4] & 1) != 0 ? 32 : 1) + (0x1F - (wiiData[3] & 0x1F))) - 32) << 10,
